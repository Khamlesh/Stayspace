  ...
  return (
    <div className="max-w-lg mx-auto animate-fade-in space-y-6">
      <div>
        ...
        <p className="text-sm text-secondary-text mt-1">Manage your host profile</p>
      </div>

      <div className="dashboard-card">
        <div className="flex items-center gap-4 mb-6">
          <div className="w-16 h-16 rounded-full bg-primary/10 flex items-center justify-center">
            <span className="text-primary font-bold text-2xl">{(form.name || 'H').charAt(0)}</span>
          </div>
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
              <span className={`text-xs px-2 py-0.5 rounded-full ${profile?.is_approved ? 'bg-success/10 text-success' : 'bg-warning/10 text-warning'}`}>
                {profile?.is_approved ? 'Approved Host' : 'Pending Approval'}
              </span>
            </div>
          </div>
        </div>
