...
import {
  HiOutlineHome, HiOutlineBuildingOffice2, HiOutlinePlusCircle,
  HiOutlineCalendarDays, HiOutlineCurrencyRupee, HiOutlineChartBar,
  HiOutlineStar, HiOutlineBell, HiOutlineUserCircle, HiOutlineCog6Tooth,
...
const menuItems = [
  { key: 'dashboard', label: 'Dashboard', icon: HiOutlineHome, path: '/host' },
  { key: 'properties', label: 'My Properties', icon: HiOutlineBuildingOffice2, path: '/host/properties' },
  { key: 'add-property', label: 'Add Property', icon: HiOutlinePlusCircle, path: '/host/add-property' },
  { key: 'bookings', label: 'Bookings', icon: HiOutlineCalendarDays, path: '/host/bookings' },
  { key: 'messages', label: 'Messages', icon: HiOutlineChatBubbleLeftRight, path: '/host/messages' },
  { key: 'earnings', label: 'Earnings', icon: HiOutlineCurrencyRupee, path: '/host/earnings' },
  { key: 'reports', label: 'Reports', icon: HiOutlineChartBar, path: '/host/reports' },
  { key: 'reviews', label: 'Reviews', icon: HiOutlineStar, path: '/host/reviews' },
  { key: 'notifications', label: 'Notifications', icon: HiOutlineBell, path: '/host/notifications' },
  { key: 'complaints', label: 'Complaints', icon: HiOutlineExclamationTriangle, path: '/host/complaints' },
  { key: 'profile', label: 'Profile', icon: HiOutlineUserCircle, path: '/host/profile' },
  { key: 'settings', label: 'Settings', icon: HiOutlineCog6Tooth, path: '/host/settings' },
]
          ...
          {!collapsed ? (
            <div className="flex items-center gap-3 px-2">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-secondary-text truncate">Host</p>
              </div>
              <button
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="h-16 bg-white border-b border-divider flex items-center px-4 gap-3 flex-shrink-0">
          ...
          <div className="flex items-center gap-2 flex-1">
            <Logo size="sm" showText={false} linkTo="/host" className="lg:hidden" />
          </div>

          <NavLink
            to="/host/add-property"
            className="hidden sm:flex items-center gap-2 px-4 py-2 bg-primary text-white text-sm font-medium rounded-xl hover:bg-primary-hover transition-colors"
          >
            <HiOutlinePlusCircle className="w-4 h-4" />
            Add Property
          </NavLink>

          <NotificationCenter apiClient={hostAPI} basePath="/host" onUnreadChange={handleUnreadChange} />

          <NavLink to="/host/profile" className="w-9 h-9 rounded-full bg-primary/10 flex items-center justify-center">
            <span className="text-primary font-semibold text-sm">{userInitial}</span>
          </NavLink>
        </header>
